...
function setStatus(status,urlId,url_string){
	//window.open("sample.htm", url_string, ""); 
	$.ajax({
		type : "POST",
		url : "updateUrlStatus",
		data :{
  ...
  <body>
    ...
    <div id="wrap">
      ...
      <div class="container" style="margin-top: 10px;">
        ...
        <font style="text-align: left;" size="3" ><strong> Approved </strong> (<a href="<%=request.getContextPath() %>/govShopVerificationLog">${userVerificationApproved }</a>) </font> <br />
        <font style="text-align: left;" size="3" ><strong> Missed </strong> (<a href="<%=request.getContextPath() %>/govShopVerificationMissed">${userVerificationAll -(userVerificationActive +userVerificationApproved)}</a>)</font> <br />
  
		<c:if test="${userVerificationActive == 0}">
        	<input type="button" name="getMoreLinks" id="getMoreLinks" class="btn btn-primary" onclick="getMoreLinks('assignGovShopData')" value="Get More 10 Links">
        </c:if>
        <br /> <br /> 
         ...
         <table class="table table-striped" >
             ...
             <c:forEach items="${urlList }" var="urlList" varStatus="index">         
	         	<tr>
	         		<td>${index.count } </td> 
	         		<td>
	         			<a  href="${urlList.url }" style="cursor: pointer;" onclick="setStatus('Done','${urlList.urlId}','${urlList.url }?id=${urlList.userId}&urlId=${urlList.urlId}')" target="_blank"> Link ${urlList.urlId }</a> 
